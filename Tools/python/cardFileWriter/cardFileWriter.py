...
class cardFileWriter:
    ...
    def calcLimit(self, fname=None, options="", normList=[], reFitOptions=None):
        import uuid, os
        ustr          = str(uuid.uuid4())
        uniqueDirname = os.path.join(self.releaseLocation, ustr)
        ...
        if reFitOptions:
            tempWS = uniqueDirname+"/ws.root"
            os.system("mv %s %s"%(tempResFile, tempWS))
            combineCommand = "cd "+uniqueDirname+";combine --saveWorkspace -M MultiDimFit %s %s"%(reFitOptions,tempWS)
            print combineCommand
            os.system(combineCommand)
...
#        return nll
    ...
    def calcNuisances(self, fname=None, options="", outputFileAddon = "", bonly=False):
        ...
        filename = os.path.abspath(fname)
        resultFilename      = filename.replace('.root','')+'_nuisances.txt'
        resultFilenameFull  = filename.replace('.root','')+'_nuisances_full.txt'
        resultFilename2     = filename.replace('.root','')+'_nuisances.tex'
        resultFilename2Full = filename.replace('.root','')+'_nuisances_full.tex'
