...
Histo = eftSample.get1DHistoFromDraw( "GenBJetCMSUnfold0_pt", binning=[20, 20, 800],selectionString=selection, weightString=smweightString, addOverFlowBin="upper" )
...
if args.parameters:
    ...
    for i_param, (coeff, val, str_val) in enumerate(zip(coeffs, vals, str_vals)):
        ...
        bsmHisto = eftSample.get1DHistoFromDraw( "GenBJetCMSUnfold0_pt", binning=[20, 20, 800], selectionString=selection, weightString=bsmweightString, addOverFlowBin="upper" )
...
def drawPlots( plots ):
    ...
    for plot in plots:
        ...
        for log in [True, False]:
            plot_directory_ = os.path.join( plot_directory, 'genPlotsEFT', str(args.year), args.plot_directory,'BJet_reweighting' , args.selection, args.mode, "log" if log else "lin" )
            plotting.draw( plot,
...
def pt_weight( event, sample ):
    if sample.name == 'SM': return
        ...
        if event.GenBJetCMSUnfold0_pt >= 800: binNumber = 20
        else: binNumber = sample.params["histo"].FindBin( event.GenBJetCMSUnfold0_pt )
