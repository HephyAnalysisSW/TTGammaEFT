...
hist = mc.get1DHistoFromDraw( "Min$(sqrt((GenJet_eta-GenPhoton_eta[0])**2+acos(cos(GenJet_phi-GenPhoton_phi[0]))**2))", binning=[10,0,1], selectionString=selection+"&&Photon_pt[0]>10&&abs(Photon_eta[0])<5", addOverFlowBin="upper" )
histdR2 = mc.get1DHistoFromDraw( "Min$(sqrt((GenJet_eta-GenPhoton_eta[0])**2+acos(cos(GenJet_phi-GenPhoton_phi[0]))**2))", binning=[10,0,1], selectionString=selection+"&&Photon_pt[0]>10&&abs(Photon_eta[0])<5"+"&&nJetGood==2", addOverFlowBin="upper" )
histdR3 = mc.get1DHistoFromDraw( "Min$(sqrt((GenJet_eta-GenPhoton_eta[0])**2+acos(cos(GenJet_phi-GenPhoton_phi[0]))**2))", binning=[10,0,1], selectionString=selection+"&&Photon_pt[0]>10&&abs(Photon_eta[0])<5"+"&&nJetGood==3", addOverFlowBin="upper" )
histdR4 = mc.get1DHistoFromDraw( "Min$(sqrt((GenJet_eta-GenPhoton_eta[0])**2+acos(cos(GenJet_phi-GenPhoton_phi[0]))**2))", binning=[10,0,1], selectionString=selection+"&&Photon_pt[0]>10&&abs(Photon_eta[0])<5"+"&&nJetGood==4", addOverFlowBin="upper" )
histdR5 = mc.get1DHistoFromDraw( "Min$(sqrt((GenJet_eta-GenPhoton_eta[0])**2+acos(cos(GenJet_phi-GenPhoton_phi[0]))**2))", binning=[10,0,1], selectionString=selection+"&&Photon_pt[0]>10&&abs(Photon_eta[0])<5"+"&&nJetGood==5", addOverFlowBin="upper" )

hist.style         = styles.lineStyle( ROOT.kRed, width=2 )
hist.legendText    = "W#gamma N_{jet}#geq2"
#hist.legendText    = "W+Jets inclusive"

histdR2.style         = styles.lineStyle( ROOT.kBlue, width=2 )
histdR2.legendText    = "W#gamma N_{jet}=2"
#histdR2.legendText    = "W+Jets min #DeltaR(gen-jet,#gamma)#leq 0.05"

histdR3.style         = styles.lineStyle( ROOT.kGreen, width=2 )
histdR3.legendText    = "W#gamma N_{jet}=3"
#histdR3.legendText    = "W+Jets min #DeltaR(gen-jet,#gamma)#leq 0.1"

histdR4.style         = styles.lineStyle( ROOT.kAzure, width=2 )
histdR4.legendText    = "W#gamma N_{jet}=4"
#histdR4.legendText    = "W+Jets min #DeltaR(gen-jet,#gamma)#leq 0.2"

histdR5.style         = styles.lineStyle( ROOT.kOrange, width=2 )
histdR5.legendText    = "W#gamma N_{jet}=5"
#histdR5.legendText    = "W+Jets min #DeltaR(gen-jet,#gamma)#leq 0.3"

histos     = [[hist], [histdR2], [histdR3], [histdR4], [histdR5]]

Plot.setDefaults()

plots = []
plots.append( Plot.fromHisto( "minPhotonJetdR", histos,        texX = "#Delta R(gen-jet,#gamma)",                   texY = "Number of Events" ) )
...
for plot in plots:
    ...
    for log in [True, False]:
        ...
        ratio = {'histos':[(1,0),(2,0),(3,0),(4,0)],'yRange':(0.1,1.9), 'logY':True }
