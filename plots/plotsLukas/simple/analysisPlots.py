...
argParser.add_argument('--plot_directory',     action='store',      default='102X_TTG_ppv43_v1')
...
def drawPlots( plots ):
    ...
    for plot in plots:
        ...
        for log in [True, False]:
            plot_directory_ = os.path.join( plot_directory, 'simplePlots', str(args.year), args.plot_directory, args.selection, args.mode, "log" if log else "lin" )
            plotting.draw( plot,
...
for sample in mc:
    ...
    sample.setSelectionString( [ filterCutMc, "triggered==1", "pTStitching==1", "overlapRemoval==1" ] )
    # you can scale the histograms of each sample by defining sample.scale (don't scale data)
    # Scale the MC histograms by the luminosity taken by CMS in each year
...
plotList.append( Plot(
    ...
    binning   = [ 20, 20, 320 ], # 20 bins from 20 to 120
))
...
plotList.append( Plot(
    ...
    texY      = 'Number of Events', # y axis label
    attribute = lambda event, sample: event.PhotonGood0_pt, # variable to plot
    binning   = [ 40, 20, 320 ], # 20 bins from 20 to 120
))

plotList.append( Plot(
    name      = 'PhotonGood0_pt_fine', # name of the plot file
    texX      = 'p_{T}(#gamma_{0}) (GeV)', # x axis label
    texY      = 'Number of Events', # y axis label
    attribute = lambda event, sample: event.PhotonGood0_pt, # variable to plot
    binning   = [ 60, 20, 320 ], # 20 bins from 20 to 120
))

plotList.append( Plot(
    name      = 'PhotonGood0_eta_coarse',
    texX      = '#eta(#gamma_{0})',
