...
def jetSelectionModifier( sys, returntype = "func"):
    ...
    variiedJetObservables = ["nJetGood", "nBTagGood", "m3", "ht"] # change that after next pp
    if returntype == "func":
...
def MC_WEIGHT( variation, lumi_scale, returntype = "string"):
    ...
    if returntype == "string":
        return "((%s)+(%s*%f*((nPhotonGood>0)*(PhotonGood0_photonCat==2))))"%(weightString,weightString,(misIDSF_val[args.year].val-1))
    if returntype == "invIso":
        weightStringInv = weightString.replace("reweightTrigger", "reweightInvTrigger")
        return "((%s)+(%s*%f*((nPhotonGoodInvLepIso>0)*(PhotonGoodInvLepIso0_photonCat==2))))"%(weightStringInv,weightStringInv,(misIDSF_val[args.year].val-1))
...
if args.variation == "central":
    ...
    if not dirDB.contains(key) or args.overwrite:
        ...
        key = (data_sample.name, "QCDBarrel", args.variable, "_".join(map(str,args.binning)), data_sample.weightString, data_sample.selectionString, preSelection_Barrel)
        if dirDB.contains(key) and not args.overwrite:
            ...
            dirDB.add(key, dataHistB_SB, overwrite=True)

        key = (data_sample.name, "QCDEC", args.variable, "_".join(map(str,args.binning)), data_sample.weightString, data_sample.selectionString, preSelection_EC)
        if dirDB.contains(key) and not args.overwrite:
            ...
            dirDB.add(key, dataHistEC_SB, overwrite=True)
        ...
        for s in mc:
            ...
            key = (s.name, "QCDBarrel", args.variable, "_".join(map(str,args.binning)), s.weightString, s.selectionString, preSelection_Barrel)
            if dirDB.contains(key) and not args.overwrite:
                ...
                dirDB.add(key, s.histB_SB, overwrite=True)

            s.setWeightString( mc_normalization_weight_string )
            key = (s.name, "QCDEC", args.variable, "_".join(map(str,args.binning)), s.weightString, s.selectionString, preSelection_EC)
            if dirDB.contains(key) and not args.overwrite:
                ...
                dirDB.add(key, s.histEC_SB, overwrite=True)
            ...
            dataHistB_SB.Add( s.histB_SB, -1 )
            dataHistEC_SB.Add( s.histEC_SB, -1 )

        dataHistB_SB.Scale(   transferFac_Barrel.val )
        dataHistEC_SB.Scale(  transferFac_EC.val )

        qcdHist = dataHistB_SB.Clone("QCD")
        qcdHist.Add(dataHistEC_SB)
        ...
        key = ("QCD-DD", "AR", args.variable, "_".join(map(str,args.binning)), selection)
        dirDB.add(key, qcdHist, overwrite=True)
