...
for index, mode in enumerate( allModes ):
    ...
    # Accounting for 
    if mode in ["e", "eetight"]:
        leptonEtaCutVar = "abs(LeptonTightInvIso0_eta+LeptonTightInvIso0_deltaEtaSC)"
    else:
        leptonEtaCutVar = "abs(LeptonTightInvIso0_eta)"

    leptonEtaCut_Barrel = leptonEtaCutVar+"<=1.479"
    leptonEtaCut_EC     = leptonEtaCutVar+">1.479"

    plots_Barrel = copy.deepcopy(plots)
    plots_EC     = copy.deepcopy(plots)
...
#    plotting.fill( plots_Barrel, read_variables=read_variables, sequence=sequence )
...
#    plotting.fill( plots_EC, read_variables=read_variables, sequence=sequence )
    ...
    if regionPlot:
        ...
        transferFac_EC     = estimate.cachedTransferFactor(mode, setup, qcdUpdates=qcdUpdates_EC,     save=True, overwrite=False, checkOnly=False)
        qcdUpdates_Barrel  = { "CR":QCDTF_updates["CRBarrel"], "SR":QCDTF_updates["SRBarrel"] }
        transferFac_Barrel = estimate.cachedTransferFactor(mode, setup, qcdUpdates=qcdUpdates_Barrel, save=True, overwrite=False, checkOnly=False)

#        transFacQCD = transFacQCD.val
        transFacQCD_EC     = transferFac_EC.val
        transFacQCD_Barrel = transferFac_Barrel.val

        print
        print
        print "EC", transFacQCD_EC
        print "Barrel", transFacQCD_Barrel
        print
        print

    else:
        if mode in ["e", "eetight"]:
            leptonSREtaCutVar = "abs(LeptonTight0_eta+LeptonTight0_deltaEtaSC)"
        else:
            leptonSREtaCutVar = "abs(LeptonTight0_eta)"

        leptonSREtaCut_Barrel = leptonSREtaCutVar+"<=1.479"
        leptonSREtaCut_EC     = leptonSREtaCutVar+">1.479"
        ...
        yield_data_CR_EC  = data_sample.getYieldFromDraw( selectionString=preSelectionCRData_EC, weightString="weight" )["val"]
        yield_data_SR_EC  = data_sample.getYieldFromDraw( selectionString=preSelectionSRData_EC, weightString="weight" )["val"]
        yield_data_CR_Barrel  = data_sample.getYieldFromDraw( selectionString=preSelectionCRData_Barrel, weightString="weight" )["val"]
        yield_data_SR_Barrel  = data_sample.getYieldFromDraw( selectionString=preSelectionSRData_Barrel, weightString="weight" )["val"]
        yield_mc_CR_EC    = 0
        yield_mc_SR_EC    = 0
        yield_mc_CR_Barrel    = 0
        yield_mc_SR_Barrel    = 0

        for sample in mc:
            sample.setSelectionString( "(1)" )
            sample.setWeightString( "(1)" )
            y_CR_EC = sample.getYieldFromDraw( selectionString=preSelectionCRMC_EC, weightString="weight*%f*%s"%(sample.scale,weightStringMisIDSF) )["val"]
            y_SR_EC = sample.getYieldFromDraw( selectionString=preSelectionSRMC_EC, weightString="weight*%f*%s"%(sample.scale,weightStringMisIDSF) )["val"]
            y_CR_Barrel = sample.getYieldFromDraw( selectionString=preSelectionCRMC_Barrel, weightString="weight*%f*%s"%(sample.scale,weightStringMisIDSF) )["val"]
            y_SR_Barrel = sample.getYieldFromDraw( selectionString=preSelectionSRMC_Barrel, weightString="weight*%f*%s"%(sample.scale,weightStringMisIDSF) )["val"]

            if addSF:
                if "DY" in sample.name:
                    y_CR_EC *= DYSF_val[setup.year] #add DY SF
                    y_SR_EC *= DYSF_val[setup.year] #add DY SF
                    y_CR_Barrel *= DYSF_val[setup.year] #add DY SF
                    y_SR_Barrel *= DYSF_val[setup.year] #add DY SF
                elif "WJets" in sample.name:
                    y_CR_EC *= WJetsSF_val[setup.year] #add WJets SF
                    y_SR_EC *= WJetsSF_val[setup.year] #add WJets SF
                    y_CR_Barrel *= WJetsSF_val[setup.year] #add WJets SF
                    y_SR_Barrel *= WJetsSF_val[setup.year] #add WJets SF
                elif "TT_pow" in sample.name:
                    y_CR_EC *= TTSF_val[setup.year] #add TT SF
                    y_SR_EC *= TTSF_val[setup.year] #add TT SF
                    y_CR_Barrel *= TTSF_val[setup.year] #add TT SF
                    y_SR_Barrel *= TTSF_val[setup.year] #add TT SF
                elif "ZG" in sample.name:
                    y_CR_EC *= ZGSF_val[setup.year] #add ZGamma SF
                    y_SR_EC *= ZGSF_val[setup.year] #add ZGamma SF
                    y_CR_Barrel *= ZGSF_val[setup.year] #add ZGamma SF
                    y_SR_Barrel *= ZGSF_val[setup.year] #add ZGamma SF
                elif "WG" in sample.name:
                    y_CR_EC *= WGSF_val[setup.year] #add WGamma SF
                    y_SR_EC *= WGSF_val[setup.year] #add WGamma SF
                    y_CR_Barrel *= WGSF_val[setup.year] #add WGamma SF
                    y_SR_Barrel *= WGSF_val[setup.year] #add WGamma SF
                elif "TTG" in sample.name:
                    y_CR_EC *= SSMSF_val[setup.year] #add SSM
                    y_SR_EC *= SSMSF_val[setup.year] #add SSM
                    y_CR_Barrel *= SSMSF_val[setup.year] #add SSM
                    y_SR_Barrel *= SSMSF_val[setup.year] #add SSM


            yield_mc_CR_EC += y_CR_EC
            yield_mc_SR_EC += y_SR_EC
            yield_mc_CR_Barrel += y_CR_Barrel
            yield_mc_SR_Barrel += y_SR_Barrel

        transFacQCD_EC = (yield_data_SR_EC - yield_mc_SR_EC) / (yield_data_CR_EC - yield_mc_CR_EC) if yield_mc_CR_EC != yield_data_CR_EC else 0.
        transFacQCD_Barrel = (yield_data_SR_Barrel - yield_mc_SR_Barrel) / (yield_data_CR_Barrel - yield_mc_CR_Barrel) if yield_mc_CR_Barrel != yield_data_CR_Barrel else 0.
        if transFacQCD_EC < 0: transFacQCD_EC = 0
        if transFacQCD_Barrel < 0: transFacQCD_Barrel = 0
        logger.info( "Inclusive TF Barrel: %f"%transFacQCD_Barrel )
        logger.info( "Inclusive TF EC: %f"%transFacQCD_EC )

    for i, plot in enumerate(plots_Barrel):
        qcdHist_Barrel  = copy.deepcopy(plot.histos[1][0])
        for h in plot.histos[0]:
            qcdHist_Barrel.Add( h, -1 )

        qcdHist_EC  = copy.deepcopy(plots_EC[i].histos[1][0])
        for h in plots_EC[i].histos[0]:
            qcdHist_EC.Add( h, -1 )

        # remove negative bins
        for i in range(qcdHist_Barrel.GetNbinsX()):
            if qcdHist_Barrel.GetBinContent(i+1) < 0:
                qcdHist_Barrel.SetBinContent(i+1, 0)

        for i in range(qcdHist_EC.GetNbinsX()):
            if qcdHist_EC.GetBinContent(i+1) < 0:
                qcdHist_EC.SetBinContent(i+1, 0)

        qcdHist_Barrel.Scale( transFacQCD_Barrel )
        qcdHist_EC.Scale( transFacQCD_EC )

        qcdHist_Barrel.Add(qcdHist_EC)
        ...
        dirDB.add( cacheName, qcdHist_Barrel, overwrite=True )
