...
def wrapper():
    ...
    cardFileNameTxt   = os.path.join( limitDir, "_".join( regionNames+EFTparams ) + ".txt" )
    cardFileNameShape = cardFileNameTxt.replace( ".txt", "_shape.root" )
    cardFileName      = cardFileNameTxt
    if ( not os.path.exists(cardFileNameTxt) or ( not os.path.exists(cardFileNameShape) and not args.useTxt ) ):
            ...
            for r in setup.regions:
                for i_ch, channel in enumerate(setup.channels):
                    ...
                    for pName, pList in setup.processes.items():
                        ...
                        if not yieldCache.contains( smyieldkey ): raise Exception("yieldCache does not contain key")
                        smyield     =  yieldCache.get(smyieldkey)["val"]

                        eftyieldkey =  (setup.name, str(r),channel, str(eft))
                        if not yieldCache.contains( eftyieldkey ): raise Exception("yieldCache does not contain key")
