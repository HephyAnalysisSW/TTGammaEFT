...
key = (data_sample.name, "MET_pt", "_".join(map(str,binning)), data_sample.weightString, data_sample.selectionString, preSelection)
...
if dirDB.contains(key) and not args.overwrite:
    ...
    dataHist_SB = data_sample.get1DHistoFromDraw( "MET_pt", binning=binning, selectionString=preSelection, addOverFlowBin="upper" )
...
key = (data_sample.name, "MET_pt", "_".join(map(str,binning)), data_sample.weightString, data_sample.selectionString, selection)
...
if dirDB.contains(key) and not args.overwrite:
    ...
    dataHist = data_sample.get1DHistoFromDraw( "MET_pt",    binning=binning, selectionString=selection,    addOverFlowBin="upper" )
...
for s in mc:
    key = (s.name, "MET_pt", "_".join(map(str,binning)), s.weightString, s.selectionString, preSelection)
#    key = (s.name, "mTinv", "_".join(map(str,binning)), s.weightString, s.selectionString, preSelection)
    if dirDB.contains(key) and not args.overwrite:
        s.hist_SB = dirDB.get(key)
    else:
        s.hist_SB = s.get1DHistoFromDraw( "MET_pt", binning=binning, selectionString=preSelection, addOverFlowBin="upper" )
        dirDB.add(key, s.hist_SB)

    key = (s.name, "MET_pt", "_".join(map(str,binning)), s.weightString, s.selectionString, selection)
#    key = (s.name, "mT", "_".join(map(str,binning)), s.weightString, s.selectionString, selection)
    if dirDB.contains(key) and not args.overwrite:
        s.hist = dirDB.get(key)
    else:
        s.hist = s.get1DHistoFromDraw( "MET_pt",    binning=binning, selectionString=selection,    addOverFlowBin="upper" )
...
elif     photonRegion and     bjetRegion and njets == 2: floatSample = tt
...
plots = []
plots.append( Plot.fromHisto( "MET_pt",          mTHistos,        texX = "E^{miss}_{T} [GeV]",                texY = "Number of Events" ) )
plots.append( Plot.fromHisto( "MET_pt_fit",      mTHistos_fit,    texX = "E^{miss}_{T} [GeV]",                texY = "Number of Events" ) )
plots.append( Plot.fromHisto( "MET_pt_sideband", mTHistos_SB,     texX = "E^{miss}_{T} (QCD sideband) [GeV]", texY = "Number of Events" ) )

for plot in plots:

    if plot.name == "MET_pt_fit":
    ...
    for log in [True, False]:
        ...
        if plot.name == "MET_pt" or plot.name == "MET_pt_fit":
            ratio = {'yRange':(0.65,1.35)}
        elif plot.name == "MET_pt_sideband":
        ...
        plotting.draw( plot,
                       ...
                       logX = False, logY = log, sorting = True,#plot.name != "MET_pt_fit",
