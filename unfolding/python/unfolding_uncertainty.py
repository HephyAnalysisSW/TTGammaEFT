...
def drawHeader():
    ...
    line = (0.18, 0.85, "MG5_aMC") 
    line2 = (0.5, 0.85, "Uncertainty")
    return [tex.DrawLatex(*line), tex.DrawLatex(*line)]

# Text on the plots
def drawObjects( offset=0, cmsOffset=0, supplementary=False ):
    tex = ROOT.TLatex()
    tex.SetNDC()
    tex.SetTextSize(0.04)
    tex.SetTextAlign(11) # align right
    ...
    line = (0.68-offset, 0.95, "%i fb{}^{-1} (13 TeV)"%settings.lumi_factor) 
#    line2 = (0.15+cmsOffset, 0.95, "CMS")
    line2 = (0.15+cmsOffset, 0.95, "Private Work")
    line3 = (0.25+cmsOffset, 0.95, "%s"%("" if not supplementary else " #bf{#it{Supplementary}}"))
...
plot_directory_ = os.path.join( plot_directory, "unfoldingJHEP", args.plot_directory+"_Herwigv4", args.settings + "_" + args.useMatrix if args.useMatrix else args.settings)
...
def draw2D( plot, logZ, supplementary, **kwargs):
    plotting.draw2D( plot,
                     plot_directory = plot_directory_,
                     logX = False, logY = False, logZ = logZ,
                     copyIndexPHP = True,  
                     drawObjects    = drawObjects(offset=0.1, supplementary=supplementary),
                     **kwargs
                    )
...
if not args.systematic is None and dirDB.contains( loop_key ) and not args.overwrite:
    ...
    fiducial_spectrum.GetXaxis().SetTitle(settings.tex_unf)
...
#else:
#    matrix = sys_matrix["nominal"]

#rootfile = ROOT.TFile( "unfoldingMatrix_%s.root"%args.settings, "RECREATE")
...
norm  = sample_pyt.getYieldFromDraw( selectionString=gen_sel, weightString="weight")["val"]
norm /= TTG_P8.getYieldFromDraw( selectionString=gen_sel, weightString="weight")["val"]
...
# quickfix
if "dRlg" in args.settings:
    genVar = "sqrt((GenPhotonCMSUnfold0_eta-GenLeptonCMSUnfold0_eta)**2+acos(cos(GenPhotonCMSUnfold0_phi-GenLeptonCMSUnfold0_phi))**2)"

gen_weight = "weight*(35.92+41.53+59.74)*%f"%norm

print "normalization P8", TTG_P8.getYieldFromDraw( selectionString=gen_sel, weightString=gen_weight )["val"]/(35.92+41.53+59.74)
fiducial_spectrum_Hpp = TTG_Hpp.get1DHistoFromDraw( genVar, binning=array.array('d', settings.fiducial_thresholds), selectionString=gen_sel, weightString=gen_weight, addOverFlowBin="upper", binningIsExplicit=True )
print "normalization Hpp", TTG_Hpp.getYieldFromDraw( selectionString=gen_sel, weightString=gen_weight )["val"]/(35.92+41.53+59.74)
fiducial_spectrum_Hpp.GetXaxis().SetTitle(settings.tex_gen)

fiducial_spectrum_H7 = TTG_H7.get1DHistoFromDraw( genVar, binning=array.array('d', settings.fiducial_thresholds), selectionString=gen_sel, weightString=gen_weight, addOverFlowBin="upper", binningIsExplicit=True )
print "normalization H7", TTG_H7.getYieldFromDraw( selectionString=gen_sel, weightString=gen_weight )["val"]/(35.92+41.53+59.74)
...
draw2D( plot_matrix, False, False, widths = {'x_width':500+250*(len(settings.years)-1)}, zRange = (0,sys_matrix["nominal"].GetMaximum()) )
plot_matrix = Plot2D.fromHisto("unfolding_matrix", [[matrix]], texY = settings.tex_gen, texX = settings.tex_reco + s_str )
draw2D( plot_matrix, True, False, widths = {'x_width':500+250*(len(settings.years)-1)}, zRange = (1,sys_matrix["nominal"].GetMaximum()) )
...
for logY in [False]:
    ...
    purity     . style =  styles.lineStyle( ROOT.kRed+1, width = 2, errors=False)
...
#rootfile = ROOT.TFile( "efficiency_%s.root"%args.settings, "RECREATE")
#purity.SetName("purity")
#purity.Write()
#efficiency.SetName("efficiency")
#efficiency.Write()
#reco_spectrum_subtracted.SetName("foutSubtracted")
#reco_fout_spectrum.SetName("fout")
#reco_spectrum.SetName("reco")
#reco_spectrum_subtracted.Write()
#reco_fout_spectrum.Write()
#reco_spectrum.Write()
#rootfile.Close()
#sys.exit()
...
def replaceMatrixUncertainty(matrix, ref):
    for ix in range(1, matrix.GetNbinsX()+1 ):
        for iy in range(1, matrix.GetNbinsY()+1 ):
            matrix.SetBinError(ix,iy, ref.GetBinError(ix,iy))
    return matrix


unfold               = {key:ROOT.TUnfold( replaceMatrixUncertainty(sys_matrix[key],sys_matrix["nominal"]), mapping, regMode, constraintMode) for key in all_systematics}
...
scan_tgraph.SetLineColor(ROOT.kRed+2)
scan_tgraph.Draw()
scan_tgraph.GetXaxis().SetTitle("log(#tau)")
scan_tgraph.GetYaxis().SetTitle("Avg. corr. coefficient")

scan_tgraph.GetXaxis().SetTitleSize(0.05)
scan_tgraph.GetXaxis().SetLabelSize(0.04)
scan_tgraph.GetYaxis().SetTitleSize(0.05)
scan_tgraph.GetYaxis().SetLabelSize(0.04)

#scan_tgraph_best.SetLineColor(ROOT.kRed+1)
#scan_tgraph_best.SetMarkerColor(ROOT.kRed+1)
...
leg.AddEntry(scan_tgraph_best, '#tau_{min} = %6.4f'% 10**best_logtau, 'P')
leg.Draw()
...
hs = []
colors = [ROOT.kBlue, ROOT.kRed+1, ROOT.kGreen+1]
for i_tau, tau in enumerate( [  0, 10**best_logtau] ):
    h = getOutput(unfold_nom_sys, reco_spectrum_subtracted, tau=tau)
    h.Scale(1./settings.lumi_factor)
    h.legendText = ( "#tau = 0" if i_tau == 0 else "#tau = %6.4f"%10**best_logtau )
    h.style = styles.lineStyle( colors[i_tau], width=3 )
    hs.append( h )
for logY in [True, False]:
    plot = Plot.fromHisto( name = 'unfolding_tau_comparison' + ('_log' if logY else ''),  histos = [[ h ] for h in hs], texX = settings.tex_unf, texY = settings.texY )
    plot.stack = None
    draw(plot, logY = logY,
         ratio = {'yRange': (0.92, 1.08), 'texY':'#tau_{min}/#tau=0', 'histos':[(1,0)]},
         yRange = (41,250), #settings.y_range,
         legend  = [ (0.60,0.6,0.9,0.91), 1 ],
        )
...
for logY in [True, False]:
    ...
    unfolded_mc_spectrum_ . style =  styles.lineStyle( ROOT.kRed+1, width = 2, errors=True)
...
empties = []
statempty = empty.Clone()
statempty.style = styles.fillStyle(ROOT.kGray, lineWidth=0)
statempty.legendText = "Stat."
statempty.legendOption = "f"
empties.append(statempty)

totempty = empty.Clone()
totempty.style = styles.fillStyle(ROOT.kGray+1, lineWidth=0)
totempty.legendText = "Total"
totempty.legendOption = "f"
empties.append(totempty)

thempty = empty.Clone()
thempty.style = styles.hashStyle(hashCode=3635, lineColor=ROOT.kRed+1, lineWidth=1, color=ROOT.kRed+1)
thempty.legendText = "Theory"
thempty.legendOption = "f"
empties.append(thempty)

headerMG = empty.Clone()
headerMG.style = styles.lineStyle(color=ROOT.kWhite, width=0)
headerMG.legendText = "MG5_aMC"
empties.append(headerMG)

headerUnc = empty.Clone()
headerUnc.style = styles.lineStyle(color=ROOT.kWhite, width=0)
headerUnc.legendText = "Uncertainty"
empties.append(headerUnc)

h7empty = empty.Clone()
h7empty.style = styles.lineStyle( ROOT.kBlack, width = 3, dashed=True)
h7empty.legendText = "Herwig7"
empties.append(h7empty)

hppempty = empty.Clone()
hppempty.style = styles.lineStyle( ROOT.kGreen+2, width = 3, dotted=True)
hppempty.legendText = "Herwig++"
empties.append(hppempty)

p8empty = empty.Clone()
p8empty.style = styles.lineStyle( ROOT.kRed+1, width = 3)
p8empty.legendText = "Pythia8"
empties.append(p8empty)

dataempty = empty.Clone()
dataempty.style = styles.errorStyle(ROOT.kBlack)
dataempty.legendText = "Observed"
empties.append(dataempty)

legEmpties = [headerMG, headerUnc, p8empty, statempty, h7empty, totempty, hppempty, thempty, dataempty]
#legEmpties = [p8empty, statempty, h7empty, totempty, hppempty, thempty, dataempty]
...
settings.unfolding_signal_input.legendOption = "lpf"
...
for logY in [True, False]:
    ...
    plotting.draw( plot, 
        ...
        ratio = {'yRange': settings.y_range_ratio, 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes+[ratio_line]} ,
...
unfolding_signal_input_subtracted.legendOption = "lpf"
reco_spectrum_subtracted.style = styles.lineStyle( ROOT.kRed+1, width = 2)
...
for logY in [True, False]:
    plotting.draw(
        ...
        legend         = [ (0.20,0.75,0.9,0.91), 2 ],
        yRange = settings.y_range,
        ratio = {'yRange': (0.3, 1.7), 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_sb+[ratio_line]},
...
#####
# stress test
nominal_input = unfolding_signal_input_subtracted.Clone()
unfolded_nominal = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], nominal_input, "unfolded_nominal").Clone()
unfolded_nominal.Scale(1./settings.lumi_factor)
unfolded_nominal.style = styles.lineStyle( ROOT.kBlack, width = 3 )
unfolded_nominal.legendText = "nominal"

nreco = int(nominal_input.GetNbinsX()/3.)
p2_input = unfolding_signal_input_subtracted.Clone()
for i in range(nreco):
    pt = nominal_input.GetBinLowEdge(i+1)
    fac = 1+0.001*(pt-20)
    print pt, fac
    p2_input.SetBinContent(i+1, p2_input.GetBinContent(i+1)*fac)
    p2_input.SetBinContent(nreco+i+1, p2_input.GetBinContent(nreco+i+1)*fac)
    p2_input.SetBinContent(2*nreco+i+1, p2_input.GetBinContent(2*nreco+i+1)*fac)
unfolded_p2 = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], p2_input, "unfolded_p2").Clone()
unfolded_p2.Scale(1./settings.lumi_factor)
unfolded_p2.style = styles.lineStyle( ROOT.kRed, width = 2 )
unfolded_p2.legendText = "Scale = 0.1%s * (p^{reco}_{T}-20 GeV)"%"%"

p4_input = unfolding_signal_input_subtracted.Clone()
for i in range(nreco):
    pt = nominal_input.GetBinLowEdge(i+1)
    fac = 1+0.002*(pt-20)
    p4_input.SetBinContent(i+1, p4_input.GetBinContent(i+1)*fac)
    p4_input.SetBinContent(nreco+i+1, p4_input.GetBinContent(nreco+i+1)*fac)
    p4_input.SetBinContent(2*nreco+i+1, p4_input.GetBinContent(2*nreco+i+1)*fac)
unfolded_p4 = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], p4_input, "unfolded_p4").Clone()
unfolded_p4.Scale(1./settings.lumi_factor)
unfolded_p4.style = styles.lineStyle( ROOT.kBlue, width = 2 )
unfolded_p4.legendText = "Scale = 0.2%s * (p^{reco}_{T}-20 GeV)"%"%"

# closure 
for logY in [True, False]:
    plot = Plot.fromHisto( name = 'unfolding_stressTest' + ('_log' if logY else ''),  histos = [[ h ] for h in [unfolded_nominal, unfolded_p2, unfolded_p4]], texX = settings.tex_unf, texY = settings.texY )
    plot.stack = None

    plotting.draw(plot,
        plot_directory = plot_directory_,
        logX = False, logY = logY, sorting = False, 
        legend         = (0.22,0.63,0.83,0.9),
        ratio = {'yRange': (0.51, 1.49), 'texY':'Scaled/Nominal', 'histos':[(1,0),(2,0)]},
        drawObjects = drawObjects(cmsOffset=0.03),
    )
...
# unfolding the data
unfolding_signal_output = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], unfolding_signal_input_subtracted, "unfolding_signal_input_subtracted_x").Clone()
unfolding_signal_output.Scale(1./settings.lumi_factor)
#unfolding_signal_output.style = styles.lineStyle( ROOT.kBlue, width = 2)
unfolding_signal_output.style = styles.errorStyle( ROOT.kBlack, width = 0 )
#unfolding_signal_output.legendText = settings.signal_legendText 
unfolding_signal_output.legendOption = "lpf"

fiducial_spectrum.Scale(1./settings.lumi_factor)
fiducial_spectrum.style = styles.lineStyle( ROOT.kRed+1, width = 3)
#fiducial_spectrum.legendText = "Pythia8"
...
fiducial_spectrum_H7.style = styles.lineStyle( ROOT.kBlack, width = 3, dashed=True)
#fiducial_spectrum_H7.legendText = "Herwig7"

fiducial_spectrum_Hpp.Scale(1./settings.lumi_factor)
fiducial_spectrum_Hpp.style = styles.lineStyle( ROOT.kGreen+2, width = 3, dotted=True)
#fiducial_spectrum_Hpp.style = styles.lineStyle( ROOT.kAzure-3, width = 3, dashed=True)
#fiducial_spectrum_Hpp.legendText = "Herwig++"
...
for band in [settings.unfolding_data_input_systematic, settings.unfolding_signal_input_MCStat]:
#for band in [settings.unfolding_signal_input_MCStat]:
    band['ref_subtracted']  = fout_subtraction(band['ref'])
    band['down_subtracted'] = fout_subtraction(band['down'])
    band['up_subtracted']   = fout_subtraction(band['up'])
...
#    band['down_unfolded']  = getOutput(unfold[band['matrix'] if not args.useMatrix else args.useMatrix], band['down_subtracted'],  "unfolding_%sdown_output"%band['name'])
#    band['up_unfolded']  = getOutput(unfold[band['matrix'] if not args.useMatrix else args.useMatrix], band['up_subtracted'],  "unfolding_%sup_output"%band['name'])
...
#for band in settings.unfolding_signal_input_systematics + settings.unfolding_signalStat_input_systematics + [settings.unfolding_data_input_systematic]:
  ...
  for band in settings.unfolding_signal_input_systematics + [settings.unfolding_signal_input_MCStat]:
    ...
    for logY in [True, False]:
        ...
        plotting.draw( plot, 
            ...
            legend         = [ (0.20,0.75,0.9,0.91), 2 ],
            yRange = settings.y_range,
            ratio = {'yRange': (0.91, 1.09), 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_in+[ratio_line]} ,
  ...
  for band in settings.unfolding_signal_input_systematics + [settings.unfolding_signal_input_MCStat]:
    ...
    for logY in [True, False]:
        plotting.draw(
            ...
            legend         = [ (0.20,0.75,0.9,0.91), 2 ],
            yRange = settings.y_range,
            ratio = {'yRange': (0.91, 1.09), 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_sub+[ratio_line]},
...
if args.extended:
  for band in settings.unfolding_signal_input_systematics:
#  for band in settings.unfolding_signalStat_input_systematics:
...
#            legend         = (0.20,0.75,0.9,0.91),
            yRange = settings.y_range,
#            ratio = {'yRange': (0.91, 1.09), 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_unf+[ratio_line]},
...
if "RunII" in args.settings:
    ...
    output_covMatrixStat = fillCovarianceHisto( copy.deepcopy(systHistsStat), copy.deepcopy(diagHistos), settings.corrFitObjStat, output_covMatrixStat )
    output_covMatrixStat.GetZaxis().SetTitle("Covariance [fb^{2}]")
    output_covMatrixStat.SetMarkerSize(1.8)
    output_covMatrixStat.GetZaxis().SetTitleOffset( 0.9 )

    for i in range( 1, output_covMatrixStat.GetNbinsX()+1 ): 
        for j in range( 1, output_covMatrixStat.GetNbinsY()+1 ): 
            if output_covMatrixStat.GetBinContent(i,j)<settings.covZRangeStat[0]:
                output_covMatrixStat.SetBinContent(i,j, settings.covZRangeStat[0])

    plot_covMatrixStat = Plot2D.fromHisto("covMatrix_stat_lin", [[output_covMatrixStat]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#    plot_covMatrixStat.drawOption = "COLZTEXT"
    draw2D( plot_covMatrixStat, False, supplementary=True )
    zlow, zhigh = settings.covZRangeStat
    output_covMatrixStat.GetZaxis().SetRangeUser( zlow, zhigh )
    plot_covMatrixStat = Plot2D.fromHisto("covMatrix_stat", [[output_covMatrixStat]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#    plot_covMatrixStat.drawOption = "COLZTEXT"
    draw2D( plot_covMatrixStat, True, supplementary=True )

    mcStatUp = unfolding_signal_output.Clone()
    mcStatDown = unfolding_signal_output.Clone()
    for i in range(unfolding_signal_output.GetNbinsX()):
        mcStatUp.SetBinContent( i+1, mcStatUp.GetBinContent(i+1) + sqrt(output_covMatrixStat.GetBinContent(i+1,i+1)))
        mcStatDown.SetBinContent( i+1, mcStatDown.GetBinContent(i+1) - sqrt(output_covMatrixStat.GetBinContent(i+1,i+1)))
...
diagHistos = [total_Matrix] #, stat_unfolded]
...
output_covMatrix.SetMarkerSize(1.8)
output_covMatrix.GetZaxis().SetTitleOffset( 0.9 )

for i in range( 1, output_covMatrix.GetNbinsX()+1 ): 
    for j in range( 1, output_covMatrix.GetNbinsY()+1 ): 
        if output_covMatrix.GetBinContent(i,j)<settings.covZRange[0]:
            output_covMatrix.SetBinContent(i,j, settings.covZRange[0])

plot_covMatrix = Plot2D.fromHisto("covMatrix_lin", [[output_covMatrix]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#plot_covMatrix.drawOption = "COLZTEXT"
draw2D( plot_covMatrix, False,  supplementary=True )
zlow, zhigh = settings.covZRange
output_covMatrix.GetZaxis().SetRangeUser( zlow, zhigh )
plot_covMatrix = Plot2D.fromHisto("covMatrix", [[output_covMatrix]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#plot_covMatrix.drawOption = "COLZTEXT"
draw2D( plot_covMatrix, True,  supplementary=True )

rootfile = ROOT.TFile( "covMatrix_%s.root"%args.settings, "RECREATE")
output_covMatrix.Write()
rootfile.Close()
...
for i in range( 1, output_corr_matrix.GetNbinsX()+1 ): 
    print output_covMatrix.GetBinContent(i,i), sqrt(output_covMatrix.GetBinContent(i,i)), unfolding_signal_output.GetBinContent(i), sqrt(output_covMatrix.GetBinContent(i,i))/unfolding_signal_output.GetBinContent(i)
    for j in range( 1, output_corr_matrix.GetNbinsY()+1 ): 
        if output_corr_matrix.GetBinContent(i,i)!=0 and output_corr_matrix.GetBinContent(j,j)!=0:
            output_corr_matrix.SetBinContent(i, j, round(100*output_covMatrix.GetBinContent(i,j)/sqrt(output_covMatrix.GetBinContent(i,i)*output_covMatrix.GetBinContent(j,j)),1))
        else:
            output_corr_matrix.SetBinContent(i, j, 0)

#output_corr_matrix.Scale(100)
output_corr_matrix.SetMarkerSize(1.8)
output_corr_matrix.GetZaxis().SetRangeUser( 0, 100 )
output_corr_matrix.GetZaxis().SetTitleOffset( 0.9 )
plot_corr_matrix = Plot2D.fromHisto("corrMatrix", [[output_corr_matrix]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Correlation [%s]  "%"%" )
plot_corr_matrix.drawOption = "COLZTEXT"
draw2D( plot_corr_matrix, False, False )

rootfile = ROOT.TFile( "corrMatrix_%s.root"%args.settings, "RECREATE")
output_corr_matrix.Write()
rootfile.Close()
...
#    total_sys_uncertainty = sqrt(  ( ( totalUncertainty_unfolded['up'].GetBinContent(i)-unfolding_signal_output.GetBinContent(i) ) )**2 
    ...
    box.SetFillColorAlpha(unfolding_signal_output_color, 0.85)
    boxes_syst.append(copy.deepcopy(box))
    stuff.append(copy.deepcopy(box))
    ...
    if unfolding_signal_output.GetBinContent(i)!=0: 
        ...
        ratio_box.SetFillColorAlpha(unfolding_signal_output_color, 0.85)
        ratio_boxes_syst.append(copy.deepcopy(ratio_box))
        stuff.append(copy.deepcopy(ratio_box))
...
boxes_theo = []
ratio_boxes_theo = []
total_theo_uncertainty = 0.175
for i in range(1, fiducial_spectrum.GetNbinsX()+1):

    box = ROOT.TBox( fiducial_spectrum.GetXaxis().GetBinLowEdge(i),  
                     max(0, fiducial_spectrum.GetBinContent(i)*(1+settings.theory_uncertainties[i-1])),
                     fiducial_spectrum.GetXaxis().GetBinUpEdge(i),
                     fiducial_spectrum.GetBinContent(i)*(1-settings.theory_uncertainties[i-1]),
                 )
    ...
    boxes_theo.append(copy.deepcopy(box))
    stuff.append(copy.deepcopy(box))

    if fiducial_spectrum.GetBinContent(i)!=0: 
        ratio_box = ROOT.TBox( 
                              fiducial_spectrum.GetXaxis().GetBinLowEdge(i),  
                              max(settings.y_range_ratio[0], (1-settings.theory_uncertainties[i-1])-(1-fiducial_spectrum.GetBinContent(i)/unfolding_signal_output.GetBinContent(i))),
                              fiducial_spectrum.GetXaxis().GetBinUpEdge(i),
                              min(settings.y_range_ratio[1], (1+settings.theory_uncertainties[i-1])-(1-fiducial_spectrum.GetBinContent(i)/unfolding_signal_output.GetBinContent(i))),
                   )
        ratio_box.SetLineColor(ROOT.kRed+1)
        ratio_box.SetFillStyle(3635)
        ratio_box.SetFillColor(ROOT.kRed+1)
        ratio_boxes_theo.append(copy.deepcopy(ratio_box))
        stuff.append(copy.deepcopy(ratio_box))
...
unfolding_systematic.style = styles.lineStyle(ROOT.kRed+1, errors=True) 
...
#    if band['name'] == 'total':
#    band['up_unfolded']   = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], band['up_subtracted'],   "band_%s_up_unfolded"%band['name'])
#    band['down_unfolded'] = getOutput(unfold['nominal' if not args.useMatrix else args.useMatrix], band['down_subtracted'], "band_%s_down_unfolded"%band['name'])
#    else:
...
#            total_sys_uncertainty = sqrt( (0.5*(band['up_unfolded'].GetBinContent(i)-band['down_unfolded'].GetBinContent(i)))**2
            total_sys_uncertainty = sqrt( ((band['up_unfolded'].GetBinContent(i)-unfolding_signal_output.GetBinContent(i)))**2
#                                     + abs(stat_unfolded["up"].GetBinContent(i)-stat_unfolded["ref"].GetBinContent(i))**2
                                     + ( total_Matrix_uncertainty.GetBinContent(i)**2 ) )
...
#                                         + abs(stat_unfolded["up"].GetBinContent(i)-stat_unfolded["ref"].GetBinContent(i))**2
                ...
                ratio_box = ROOT.TBox( 
                                 unfolding_signal_output.GetXaxis().GetBinLowEdge(i),  
                                 max(0, unfolding_signal_output.GetBinContent(i) - total_sys_uncertainty)/unfolding_signal_output.GetBinContent(i),
                                 unfolding_signal_output.GetXaxis().GetBinUpEdge(i),
                                 (unfolding_signal_output.GetBinContent(i) + total_sys_uncertainty)/unfolding_signal_output.GetBinContent(i),
                     )
...
for logY in [True,False]:
    ...
    plotting.draw(plot,
        ...
        legend         = [ (0.20,0.75,0.9,0.91), 2 ],
        yRange = settings.y_range,
        ratio = {'yRange': settings.y_range_ratio, 'texY':'Pred./Obs.', 'histos':[(0,3),(1,3),(2,3)], 'drawObjects':ratio_boxes_uf+[ratio_line]} ,
#        ratio = {'yRange': settings.y_range_ratio, 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_uf+[ratio_line]} ,
...
rootfile = ROOT.TFile( "unfolded_%s.root"%args.settings, "RECREATE")
fiducial_spectrum_H7.SetName("fiducial_spectrum_H7")
fiducial_spectrum_H7.Write()
fiducial_spectrum_Hpp.SetName("fiducial_spectrum_Hpp")
fiducial_spectrum_Hpp.Write()
fiducial_spectrum.SetName("fiducial_spectrum")
fiducial_spectrum.Write()
unfolding_signal_output.SetName("unfoled_spectrum")
unfolding_signal_output.Write()
totalUncertainty_unfolded['up'].SetName("totalUncertaintySource_up")
totalUncertainty_unfolded['up'].Write()
totalUncertainty_unfolded['down'].SetName("totalUncertaintySource_down")
totalUncertainty_unfolded['down'].Write()
mcStat_unfolded['up'].SetName("mcStat_up")
mcStat_unfolded['up'].Write()
mcStat_unfolded['down'].SetName("mcStat_down")
mcStat_unfolded['down'].Write()
total_Matrix_uncertainty.SetName("totalMatrixVariationUnc_up")
total_Matrix_uncertainty.Write()
stat_unfolded['up'].SetName("stat_up")
stat_unfolded['up'].Write()
stat_unfolded['down'].SetName("stat_down")
stat_unfolded['down'].Write()

for band in reversed(settings.unfolding_signal_input_systematic_bands):
    if band['name'] == 'total':
       band['up_unfolded'].SetName("totalUncertainty_up")
       band['up_unfolded'].Write()
       band['down_unfolded'].SetName("totalUncertainty_down")
       band['down_unfolded'].Write()
...
marker = unfolding_signal_output.Clone("mark")
marker.style = styles.errorStyle(ROOT.kBlack)
marker.notInLegend = True

fiducial_spectrum.notInLegend = True
fiducial_spectrum_H7.notInLegend = True
fiducial_spectrum_Hpp.notInLegend = True
unfolding_signal_output.notInLegend = True

def setLeftMargin(canvas):
    tPad = canvas.cd(1)
    tPad.SetLeftMargin(0.18)
    bPad = canvas.cd(2)
    bPad.SetLeftMargin(0.18)

for i in range( marker.GetNbinsX() ):
    marker.SetBinError(i+1,0.001)
...
for logY in [True,False]:
    ...
    hist_mod += [lambda h:h.GetYaxis().SetTitleOffset(2.2)]
    canvasModifications  = []
    canvasModifications += [lambda c:setLeftMargin(c)]
#    canvasModifications += [lambda c:c.SetBottomMargin(0.25)]
#    hist_mod += [lambda h: h.GetYaxis().SetTitleSize(0.025)]
#    hist_mod += [lambda h: h.GetYaxis().SetLabelSize(0.025)]
    ...
    plotting.draw(plot,
        ...
        #legend = None,
        histModifications = hist_mod,
        legend         = [ (0.22,0.63,0.94,0.9), 2 ],
        yRange = settings.y_range,
        ratio = {'yRange': settings.y_range_ratio, 'texY':'Pred./Obs.', 'histos':[(0,3),(1,3),(2,3)], 'drawObjects':ratio_boxes_theo+ratio_boxes_syst+[ratio_line]} ,
#        ratio = {'yRange': settings.y_range_ratio, 'texY':'Pred./Obs.', 'histos':[(0,1)], 'drawObjects':ratio_boxes_syst+[ratio_line]} ,
        drawObjects = drawObjects(cmsOffset=0.03)+boxes_theo+boxes_syst,
        #drawObjects = boxes,
...
if "RunII" in args.settings:
    ...
    for i in range( 1, output_corr_matrixStat.GetNbinsX()+1 ): 
        print "stat", output_corr_matrixStat.GetBinContent(i,i), sqrt(output_corr_matrixStat.GetBinContent(i,i)), unfolding_signal_output.GetBinContent(i), sqrt(output_corr_matrixStat.GetBinContent(i,i))/unfolding_signal_output.GetBinContent(i)
    ...
    output_corr_matrixStat.SetMarkerSize(1.8)
    output_corr_matrixStat.GetZaxis().SetTitleOffset( 0.9 )
    output_corr_matrixStat.GetZaxis().SetRangeUser( -5, 100 )
    output_corr_matrixStat.GetZaxis().SetTitle("Correlation [%s]  "%"%")
    plot_corr_matrixStat = Plot2D.fromHisto("corrMatrix_stat", [[output_corr_matrixStat]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Correlation [%s]  "%"%" )
    plot_corr_matrixStat.drawOption = "COLZTEXT"
    draw2D( plot_corr_matrixStat, False, True )



    rootfile = ROOT.TFile( "covMatrix_stat_%s.root"%args.settings, "RECREATE")
    output_covMatrixStat.Write()
    rootfile.Close()

    rootfile = ROOT.TFile( "corrMatrix_stat_%s.root"%args.settings, "RECREATE")
    output_corr_matrixStat.Write()
    rootfile.Close()
    ...
    output_covMatrixSyst.SetMarkerSize(1.8)
    output_covMatrixSyst.GetZaxis().SetTitleOffset( 0.9 )

    for i in range( 1, output_covMatrixSyst.GetNbinsX()+1 ): 
        for j in range( 1, output_covMatrixSyst.GetNbinsY()+1 ): 
            if output_covMatrixSyst.GetBinContent(i,j)<settings.covZRange[0]:
                output_covMatrixSyst.SetBinContent(i,j, settings.covZRange[0])

    plot_covMatrixSyst = Plot2D.fromHisto("covMatrix_syst_lin", [[output_covMatrixSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#    plot_covMatrixSyst.drawOption = "COLZTEXT"
    draw2D( plot_covMatrixSyst, False,  supplementary=True )
    zlow, zhigh = settings.covZRange
    output_covMatrixSyst.GetZaxis().SetRangeUser( zlow, zhigh )
    plot_covMatrixSyst = Plot2D.fromHisto("covMatrix_syst", [[output_covMatrixSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
#    plot_covMatrixSyst.drawOption = "COLZTEXT"
    draw2D( plot_covMatrixSyst, True,  supplementary=True )
    ...
    for i in range( 1, output_corr_matrixSyst.GetNbinsX()+1 ): 
        print "syst", output_corr_matrixSyst.GetBinContent(i,i), sqrt(output_corr_matrixSyst.GetBinContent(i,i)), unfolding_signal_output.GetBinContent(i), sqrt(output_corr_matrixSyst.GetBinContent(i,i))/unfolding_signal_output.GetBinContent(i)
    ...
    output_corr_matrixSyst.SetMarkerSize(1.8)
    output_corr_matrixSyst.GetZaxis().SetTitleOffset( 0.9 )
    output_corr_matrixSyst.GetZaxis().SetRangeUser( 0, 100 )
    output_corr_matrixSyst.GetZaxis().SetTitle("Correlation [%s]"%"%")
    plot_corr_matrixSyst = Plot2D.fromHisto("corrMatrix_syst", [[output_corr_matrixSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Correlation [%s]  "%"%" )
    plot_corr_matrixSyst.drawOption = "COLZTEXT"
    draw2D( plot_corr_matrixSyst, False, False )


    rootfile = ROOT.TFile( "covMatrix_syst_%s.root"%args.settings, "RECREATE")
    output_covMatrixSyst.Write()
    rootfile.Close()

    rootfile = ROOT.TFile( "corrMatrix_syst_%s.root"%args.settings, "RECREATE")
    output_corr_matrixSyst.Write()
    rootfile.Close()
...
#        diag  = (mcStat_unfolded["up"].GetBinContent(i) - mcStat_unfolded["ref"].GetBinContent(i))**2
    ...
    plot_covMatrixTrueSyst = Plot2D.fromHisto("covMatrix_truesyst_lin", [[output_covMatrixTrueSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
    draw2D( plot_covMatrixTrueSyst, False, supplementary=True )
    zlow, zhigh = settings.covZRange
    output_covMatrixTrueSyst.GetZaxis().SetRangeUser( zlow, zhigh )
    plot_covMatrixTrueSyst = Plot2D.fromHisto("covMatrix_truesyst", [[output_covMatrixTrueSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Covariance [fb^{2}]" )
    draw2D( plot_covMatrixTrueSyst, True, supplementary=True )
    ...
    output_corr_matrixTrueSyst.GetZaxis().SetRangeUser( 0, 100 )
    output_corr_matrixTrueSyst.GetZaxis().SetTitle("Correlation [%s]"%"%")
    plot_corr_matrixTrueSyst = Plot2D.fromHisto("corrMatrix_truesyst", [[output_corr_matrixTrueSyst]], texY = settings.tex_unf, texX = settings.tex_unf, texZ="Correlation [%s]  "%"%" )
    plot_corr_matrixTrueSyst.drawOption = "COLZTEXT"
    draw2D( plot_corr_matrixTrueSyst, False, False )
...
#        print output_covMatrixTheory.GetBinContent(i,i), sqrt(output_covMatrixTheory.GetBinContent(i,i)), sqrt(output_covMatrixTheory.GetBinContent(i,i))/fiducial_spectrum.GetBinContent(i)
    ...
    draw2D( plot_covMatrixTheory, False, False )
    zlow, zhigh = settings.covZRange
    ...
    draw2D( plot_covMatrixTheory, True, False )
...
#            print i,j,invfullTheoryCovMatrix.GetBinContent(i,j)
    ...
    draw2D( plot_covMatrixInvFull, False, False )
#    zlow, zhigh = settings.covZRange
    ...
    draw2D( plot_covMatrixInvFull, True, False )
    ...
    for i in range( 1, unfolding_signal_output.GetNbinsX()+1 ):
        for j in range( 1, unfolding_signal_output.GetNbinsX()+1 ):
            ...
            yihpp = unfolding_signal_output.GetBinContent(i) - fiducial_spectrum_Hpp.GetBinContent(i)
            yjhpp = unfolding_signal_output.GetBinContent(j) - fiducial_spectrum_Hpp.GetBinContent(j)

            yih7 = unfolding_signal_output.GetBinContent(i) - fiducial_spectrum_H7.GetBinContent(i)
            yjh7 = unfolding_signal_output.GetBinContent(j) - fiducial_spectrum_H7.GetBinContent(j)

            invCov = invfullTheoryCovMatrix.GetBinContent(i, j)
#            print i,j,yi,yj,invCov, yi * invCov * yj
            chi2 += yi * invCov * yj
            chi2Hpp += yihpp * invCov * yjhpp
            chi2H7 += yih7 * invCov * yjh7

    print
    print
    print "Chi2 Pythia", chi2
    print "Chi2 Herwigpp", chi2Hpp
    print "Chi2 Herwig7", chi2H7
    print
    print
...
draw2D( plot_probability_matrix, True, False )
...
plot_output_covariance_matrix = Plot2D.fromHisto("output_covariance_matrix", [[output_covariance_matrix]], texY = settings.tex_unf, texX = settings.tex_unf )
draw2D( plot_output_covariance_matrix, True, False )
...
output_correlation_matrix.GetZaxis().SetTitle("Correlation [%s]"%"%")
output_correlation_matrix.Scale(100)
output_correlation_matrix.GetZaxis().SetRangeUser( 0, 100 )
plot_output_correlation_matrix = Plot2D.fromHisto("output_correlation_matrix", [[output_correlation_matrix]], texY = settings.tex_unf, texX = settings.tex_unf )
draw2D( plot_output_correlation_matrix, False, False )
...
draw2D( plot_input_inverse_covariance_matrix, True, False )
...
draw2D( plot_input_covariance_matrix, True, False )
...
input_correlation_matrix.GetZaxis().SetRangeUser( -0.2, 1 )
...
draw2D( plot_input_correlation_matrix, False, False )
